...
对训练的实在后果：决策期屏幕无人看，合成还在刷，**CPU 被无效渲染占住**。整机密度有自己的数：刷新 **60fps → 20fps**，同机并发 **+20%～25%**。这是全局降帧，**没有拆截图 / 非截图**，不能拿对标 2 核反推。

## 2. CPU 渲染对业务问题的影响（按真实训练规模量化）

### 2.1 非截图阶段：占用用热点，密度用整机降帧，两件事不要并成一条因果

决策往往占逐步大部分墙钟，此时不需要出给 Agent 的新帧，合成仍在刷。内部画像：复杂界面软渲染 **CPU 热点超过 50%**。这一条只证明 **无效渲染在吃核**。

密度不要用对标 2 核。catalog 4 核可能叠了 MicroVM / Android 常驻 / 1080p / 6GiB / 配额保守，**尚未拆开**。DigiRL / MobileGUI-RL 的 2 核只说明行业有人这样装。

自己测过的密度是：**整机把刷新从 60fps 降到 20fps，并发提升 20%～25%。** 含义：维持 60fps（按给人看的节奏刷）相对 20fps，同机路数少开这一截。反过来说，密度约为 20fps 时的 80%～83%。

这一刀是 **全局帧率帽**，截图期和非截图期一起降。装箱可能卡峰值也可能卡均值，所以：

- 可以当「渲染占核会拖密度」的 **整机量级：20%～25%**
- **不能**写成「已经证明这 20%～25% 全是非截图阶段」
- **不能**拆给按需或快速各自多少
- 按需（决策期不刷）比「全程 20fps 还在刷」更狠，收益 **有可能** 大于 20%～25%，但没分阶段对照，不能把更大的数填进去

两条手段各自改哪一段：

- **按需：** 切决策期无效刷新。密度贡献尚未单独测，整机 60→20 只给量级。
- **快速：** 切截图期出帧。60→20 会让单帧从 16.7ms 变成 50ms，和 &lt;10fps 等截图不是同一档。

**给决策的一句话：** 渲染刷新从 60 降到 20，整机密度 +20%～25%；未分阶段，不要记成非截图专属，更不要用对标 2 核改成 50%。
...
| 非截图阶段占用 CPU | **超过 50%**（热点占比） | 内部画像，复杂界面软渲染 | 已等于密度 20%～25% |
| 单机并发密度变少 | **20%～25%** | 整机刷新 60fps→20fps，并发 +20%～25% | 对标 2/4=50%；「已证明全是非截图阶段」 |

一轮增幅与步数无关：ΔT/T = Δt_截图 / T_单步。&lt;10fps 一帧 ≥100ms，相对 60fps 多等约 0.1～0.3s（尾部才到 1s）。单步按环境+中小模型推理 2～3s 计，得 5%～15%。

密度 20%～25% 是 **全局降帧的整机增益**，填「维持 60fps 相对 20fps 少开的那一截」。表头若写「非截图阶段导致密度变少」，口头补一句：**实验未分阶段**，这是渲染占核的整机量级，不是非截图专属。要拆按需 / 快速，需要「只在决策期降帧、截图期保持高帧」的对照。
