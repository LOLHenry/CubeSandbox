...
### 2.3 对外填表口径（只有这两个百分数能写）

| 填空 | 可写的数 | 依据 | 不能写成 |
| --- | --- | --- | --- |
| 一轮 Rollout 时间增加 | **约 5%～15%**（推荐填 **约 10%**） | 见下式；未测逐步墙钟 | 50%、翻倍；也不是热点 50% 直接搬过来 |
| 单机并发密度变少 | **约 50%**（降到对标一半） | catalog 4 核 / 对标 2 核 | 已测装箱实验；内存先卡住时还不成立 |

一轮增幅与步数无关：ΔT/T = Δt_截图 / T_单步。&lt;10fps 一帧 ≥100ms，相对 60fps 多等约 0.1～0.3s（尾部才到 1s）。单步按环境+中小模型推理 2～3s 计，得 5%～15%。推理更长则百分数更小；循环里若已有 1～2s 固定等待，增幅会被吞掉、接近 0。密度 50% 来自装箱核数，不是帧率。
