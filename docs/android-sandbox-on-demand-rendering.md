...
**业务逻辑：不该送的工还在送 → 按需。** 训练循环只要截图那一张，系统仍按给人看的方式刷。决策期 60fps、App 画完 SurfaceFlinger 再 GLES 叠（guest 下 HWC 弱，**同一屏打两次** SwiftShader）、WebView/游戏自行送帧、catalog **1080×1920** 每帧像素多——这些都不改 SwiftShader 源码，改的是还让不让它接单。
...
1. **按需渲染：** 切业务逻辑。少送工（少排、少自行送帧、少 GLES 合成）。密度贡献待测。  
