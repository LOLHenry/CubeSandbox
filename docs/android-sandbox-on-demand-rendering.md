...
**给决策的一句话：** 训练每步都有当前策略前向，往往占逐步大部分墙钟；此阶段屏幕无人看，软渲染仍按给人看的方式刷，CPU 被无效占用，挤占整机超卖且截图峰值交错条件下可开的路数。表上密度不填百分数。
...
1. **按需渲染：** 训练采轨迹时，当前策略前向往往占逐步大部分墙钟。此阶段屏幕无人看，软渲染仍按给人看的方式刷，CPU 被无效占用，挤占整机超卖且截图峰值交错条件下可开的路数。  
