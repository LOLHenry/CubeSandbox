...
- **不截图时还在画：** 每步里的「决策」就是 **当前策略的前向**（训练采轨迹，不是上线 serving）。这一段往往占逐步大部分墙钟，当步观测已取、不需要新帧，合成仍按交互显示持续出帧，CPU 继续烧。

对训练的实在后果：决策期不需要新帧，合成还在刷，**CPU 被无效渲染占住**。全程 20fps 的实验里，同机并发可 **+20%～25%**；那是全程降帧，不是按需配方，不能直接当兑现密度。不能拿对标 2 核反推。
...
**给决策的一句话：** 训练每步都有当前策略前向，往往占逐步大部分墙钟；此阶段当步观测已取、不需要新帧，软渲染仍按交互显示持续出帧，CPU 被无效占用，挤占整机超卖且截图峰值交错条件下可开的路数。表上密度不填百分数。
...
**业务逻辑：不该送的工还在送 → 按需。** 训练每步都是截图 → 当前策略前向 → 动作。前向这段当步观测已取、不需要新帧，仍按交互显示持续出帧。决策期 60fps、App 画完 SurfaceFlinger 再 GLES 叠（guest 下 HWC 弱，**同一屏打两次** SwiftShader）、WebView/游戏自行送帧、catalog **1080×1920** 每帧像素多——这些都不改 SwiftShader 源码，改的是还让不让它接单。
...
| 类型 | 模块 | 干什么 | 追问口径 |
| --- | --- | --- | --- |
| 业务逻辑 | 出帧策略 | 决策期观测已取仍排帧、合成 | 按需切；只停 Choreographer 停不掉自行送帧 |
...
**汇报口播（旁注，照着念）：** 渲染改为面向人和 Agent：决策期仍出帧，按需减 CPU、提密度；像素着色慢，鲲鹏向量多线程加速出帧。

展开（被追问时用，不入口播）：决策 = 采轨迹时当前策略前向，不是上线 serving；「过半」是口头约数，表上不写死百分数；加路取决于整机超卖且截图峰值可交错，兑现密度不填；像素着色与混合对应 PixelProcessor，画像未拆模块占比；鲲鹏向量 / 多线程是官方 SIMD + 多核手段，先画像再改。

1. **按需渲染：** 训练中大模型决策往往占逐步过半时间，此阶段当步观测已取、不需要新帧，软渲染仍持续出帧；需按需减少 CPU 无效占用、提高单机并发密度。  
2. **快速渲染：** SwiftShader 卡在像素着色与混合，需用鲲鹏向量加速、多线程加快出帧。
...
VM 软渲染（`gpu_mode=guest`）按原软件栈线框把 SwiftShader 接在 **系统库**（原图 Native：应用和内核之间的 C/C++ 图形库，OpenGL ES 在这里）后面，旁路内核 GPU。线框 PPT：[`assets/android-graphics-stack-wireframe.pptx`](assets/android-graphics-stack-wireframe.pptx) 第 4 页。胶片不要写 Native，口播用系统库。
