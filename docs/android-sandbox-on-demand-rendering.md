...
一轮增幅与步数无关：ΔT/T = Δt_截图 / T_单步。&lt;10fps 相对 60fps 多等约 0.1～0.3 s（1～2 帧；1 s 为尾部，不进均值）。**T_单步 = 2～3 s** 指 2B～8B、单图前向、无队列且无固定 sleep 时，「动作 + 等稳定截图 + 推理」的约定合计墙钟，不是纯推理、也不是实测。代入得约 5%～15%（取中约 10%）；T_单步更长则百分数更小。
