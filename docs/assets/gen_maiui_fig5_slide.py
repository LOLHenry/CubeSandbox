...
def annotate_fig5(src):
    ...
    c.text((24, h + 48), "② 装箱边：沙箱节点上路数。决策在模型节点上算，沙箱仍出帧。论文 GPU Worker = 模型节点（GPU 或昇腾）。", font=font(26), fill=BLUE)
    return cap
...
def slide(annotated):
    ...
    d.text((36, 58), "左图结构对齐 MAI-UI Fig.5。论文 GPU Worker 改称模型节点（GPU 或昇腾）；CPU Worker 改称沙箱节点。①② 与右边两段对应。", font=font(18), fill=(190, 205, 230))
    ...
    d.text((rx + 80, 606), "非截图阶段 · 打在沙箱节点路数", font=font(26), fill=BLUE)
    t2 = (
        "决策期不需要新帧，软渲染仍在出帧，\n"
        "较多占用 CPU，单机并发沙箱密度减少。\n"
        "可优化空间 20%+†"
...
def schematic():
    ...
    d.text((360, 100), "模型节点  ·  Agent 循环（GPU / 昇腾）", font=font(22), fill=BLUE)
    ...
    d.text((1040, 96), "沙箱节点  ·  Android 环境（CPU）", font=font(22), fill=(46, 125, 50))
...
def write_pptx(schematic_path):
    ...
    text(
        ...
        12.7,
        0.4,
        "* 中小模型、单步约 2–3s。† 全程 60→20fps 实验上限，不是按需已兑现。模型节点 = GPU 或昇腾；沙箱节点 = CPU。论文 Fig.5 的 GPU Worker 按此口播。",
        11,
        MUTED,
    )
