...
#
# Export Android sandbox Docker images (ReDroid + envd) for Kunpeng offline install.
#
# Usage:
#   ./deploy/one-click/scripts/one-click/build-android-sandbox-envd-offline-bundle.sh
#   RELEASE_TAG=envd-preview BUILD_IMAGE=1 ./deploy/one-click/scripts/one-click/build-android-sandbox-envd-offline-bundle.sh
#
# Environment:
#   RELEASE_TAG / CUBE_VERSION  Version suffix for bundle filename
#   ANDROID_IMAGE_TAG           Image tag (default: 16.0.0-arm64)
#   BUILD_IMAGE=1               Build sandbox-android-redroid-envd image first
...
ENVD_IMAGE_DIR="${REPO_ROOT}/deploy/sandbox-images/sandbox-android-redroid-envd"
# shellcheck source=../../lib/common.sh
source "${ONE_CLICK_DIR}/lib/common.sh"

TAG="${ANDROID_IMAGE_TAG:-16.0.0-arm64}"
RELEASE_TAG="${RELEASE_TAG:-${CUBE_VERSION:-}}"
CN_IMAGE="cube-sandbox-cn.tencentcloudcr.com/cube-sandbox/sandbox-android-redroid-envd:${TAG}"
...
if [[ -n "${RELEASE_TAG}" ]]; then
  BUNDLE_NAME="${BUNDLE_NAME:-cube-sandbox-android-kunpeng-arm64-envd-docker-${RELEASE_TAG}.tar.gz}"
else
  BUNDLE_NAME="${BUNDLE_NAME:-cube-sandbox-android-kunpeng-arm64-envd-docker.tar.gz}"
fi
...
if [[ "${BUILD_IMAGE:-0}" == "1" ]]; then
  log "building android sandbox envd image (BUILD_IMAGE=1)"
  TAG="${TAG}" "${ENVD_IMAGE_DIR}/build.sh"
fi
...
if [[ -z "${SOURCE_IMAGE}" ]]; then
  die "missing android envd image; set BUILD_IMAGE=1 or run ${ENVD_IMAGE_DIR}/build.sh"
fi
...
log "offline envd bundle ready: ${OUT_PATH}"
