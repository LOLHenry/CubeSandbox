...
//
// Minimal Android/bionic starter: daemonize envd, then exec ReDroid /init.
...
func main() {
	...
	envdLog := getenv("ENVD_LOG", "/tmp/envd.log")

	// /data may not be mounted yet when PID1 runs; /tmp is always available.
	_ = os.MkdirAll("/tmp", 0o755)
	_ = os.MkdirAll("/data/local/tmp", 0o755)

	if err := startEnvd(envdBin, envdPort, envdLog); err != nil {
		fmt.Fprintf(os.Stderr, "envd-starter: %v (cube-envd init service may start envd later)\n", err)
	}
